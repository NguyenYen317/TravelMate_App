...
class _CommunityScreenState extends State<CommunityScreen> {
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('AI goi y'),
          bottom: const TabBar(
            tabs: [
              Tab(text: 'AI goi y'),
              Tab(text: 'Nhat ky cong dong'),
            ],
          ),
        ),
        body: TabBarView(
          children: [
            Consumer2<TripPlannerProvider, SearchProvider>(
              builder: (context, tripProvider, searchProvider, _) {
                ...
                final sortedPosts = [...posts]
                  ..sort((a, b) {
                    final aSaved = searchProvider.isFavorite(
                      _favoritePlaceId(a.id),
                    );
                    final bSaved = searchProvider.isFavorite(
                      _favoritePlaceId(b.id),
                    );
                    if (aSaved == bSaved) {
