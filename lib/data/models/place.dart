...
class Place {
  ...
  factory Place.fromMap(Map<String, dynamic> map) {
    return Place(
      ...
      extraTags: map['extraTags'] != null
          ? Map<String, dynamic>.from(map['extraTags'])
          : const {},
    );
  }

  factory Place.fromNominatim(Map<String, dynamic> json) {
    final addr = json['address'] != null
        ? Map<String, dynamic>.from(json['address'])
        : <String, dynamic>{};
    final tags = json['extratags'] != null
    ...
    if (addr['house_number'] != null)
      addressParts.add(addr['house_number'].toString());
    if (addr['road'] != null) addressParts.add(addr['road'].toString());
    if (addr['suburb'] != null) addressParts.add(addr['suburb'].toString());
    if (addr['city_district'] != null)
      addressParts.add(addr['city_district'].toString());
  ...
  factory Place.fromOverpass(Map<String, dynamic> json) {
    ...
    if (tags['addr:housenumber'] != null)
      addrParts.add(tags['addr:housenumber'].toString());
    if (tags['addr:street'] != null)
      addrParts.add(tags['addr:street'].toString());
    if (tags['addr:suburb'] != null)
      addrParts.add(tags['addr:suburb'].toString());
    if (tags['addr:district'] != null)
      addrParts.add(tags['addr:district'].toString());
    ...
    final category = amenity.isNotEmpty
        ? amenity
        : (tourism.isNotEmpty ? tourism : 'Địa điểm');
  ...
  static String _getSmartImage(
    ...
    if (search.contains('restaurant') ||
        search.contains('food') ||
        search.contains('ăn'))
      keyword = 'restaurant';
    else if (search.contains('hotel') ||
        search.contains('resort') ||
        search.contains('nghỉ'))
      keyword = 'hotel';
    else if (search.contains('cafe') || search.contains('coffee'))
      keyword = 'coffee';
    else if (search.contains('park') ||
        search.contains('nature') ||
        search.contains('công viên'))
      keyword = 'park';
    else if (search.contains('museum') || search.contains('history'))
      keyword = 'museum';
    else if (search.contains('beach') ||
        search.contains('sea') ||
        search.contains('biển'))
      keyword = 'beach';
